...
swing @p[tag=cnk.interact_scarecrow,distance=..20] mainhand
execute as @p[tag=cnk.interact_scarecrow,distance=..20] run item modify entity @s[gamemode=!creative] weapon.mainhand {"type":"minecraft:set_count","count":-1,"add":true}
