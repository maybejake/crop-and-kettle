...
execute if data storage cnk:temp cooking_pot.Items[{components:{"minecraft:custom_data":{cnk:{pail:{liquid:"milk"}}}}}] run scoreboard players add $milk_count cnk.dummy 1

execute if data storage cnk:temp cooking_pot.Items[{components:{"minecraft:custom_data":{cnk:{ingredient:{generic:["milk"]}}}}}] run scoreboard players add $milk_count cnk.dummy 1
