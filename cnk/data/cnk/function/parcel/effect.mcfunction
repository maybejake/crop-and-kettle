...
# statistic
scoreboard players add @s cnk.statistic.parcels_opened 1
...
execute if predicate cnk:parcel/mainhand run return run item modify entity @s[gamemode=!creative] weapon.mainhand {"type":"minecraft:set_count","count":-1,"add":true}
execute if predicate cnk:parcel/offhand run return run item modify entity @s[gamemode=!creative] weapon.offhand {"type":"minecraft:set_count","count":-1,"add":true}