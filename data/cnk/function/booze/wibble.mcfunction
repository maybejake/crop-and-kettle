execute store result score $movement_chance cnk.dummy run random value 0..1
...
execute if score $wibble_chance cnk.dummy matches 1 run data modify entity @s[tag=!cnk.wibbled] transformation.left_rotation[0] set value 0.2
execute if score $wibble_chance cnk.dummy matches 2 run data modify entity @s[tag=!cnk.wibbled] transformation.left_rotation[0] set value -0.2
