...
execute store result storage cnk:temp mulled_wine.duration int 1 run scoreboard players get $duration cnk.dummy
execute store result storage cnk:temp mulled_wine.potency int 1 run scoreboard players get $potency cnk.dummy

execute at @s run function cnk:drinks/mulled_wine/effect/effect with storage cnk:temp mulled_wine