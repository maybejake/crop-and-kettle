...
execute unless block ~ ~1 ~ #minecraft:air run return run function cnk:seeds/rice/summon {count:1}
...
execute align xyz run summon item_display ~0.5 ~0.5 ~0.5 {item_display:"ground",Tags:["cnk.rice_crop","cnk.crop","cnk.base_propagule","smithed.block","smithed.entity","smithed.strict"],transformation:{left_rotation:[0f,0f,0f,1f],right_rotation:[0f,0f,0f,1f],translation:[0f,-0.5f,0f],scale:[1.01f,1.01f,1.01f]},item:{id:"minecraft:barrier",count:1,components:{"minecraft:item_model":"cnk:rice_crop_0"}}}
