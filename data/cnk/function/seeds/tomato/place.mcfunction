...
execute unless block ~ ~ ~ #cnk:air run return run function cnk:seeds/tomato/summon {count:1}
execute unless block ~ ~1 ~ #cnk:air run return run function cnk:seeds/tomato/summon {count:1}

setblock ~ ~ ~ minecraft:wheat[age=0] replace
playsound minecraft:item.crop.plant block @a ~ ~ ~ 1 1

execute align xyz run summon item_display ~0.5 ~0.5 ~0.5 {item_display:"ground",Tags:["cnk.tomato_crop","cnk.crop","cnk.ticking_crop","cnk.base_wheat","smithed.block","smithed.entity","smithed.strict"],transformation:{left_rotation:[0f,0f,0f,1f],right_rotation:[0f,0f,0f,1f],translation:[0f,-0.5f,0f],scale:[1.01f,1.01f,1.01f]},item:{id:"minecraft:barrier",count:1,components:{"minecraft:item_model":"cnk:tomato_crop_0"}}}
