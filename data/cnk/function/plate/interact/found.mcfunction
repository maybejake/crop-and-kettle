...
execute unless items entity @p[tag=cnk.interact_plate] weapon.mainhand * on vehicle on passengers if entity @s[type=minecraft:item_display,tag=cnk.plate_item] on vehicle run return run function cnk:plate/interact/remove_last